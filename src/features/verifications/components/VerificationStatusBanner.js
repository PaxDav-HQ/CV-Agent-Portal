...
const VerificationStatusBanner = ({ data = {} }) => {
  const stepper = data.stepper || [
    { id: "1", name: "Submitted", isCompleted: true, isInProgress: false, date: "Sep 19, 2026" },
  ...
  return (
    ...
    >
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={7.5} sx={{ borderRight: { md: "1px solid #F1F5F9" }, pr: { md: 4 } }}>
          <Box sx={{ display: "flex", alignItems: "flex-start", gap: 2, mb: 3.5 }}>
            ...
            >
              {isApproved ? (
                <VerifiedUserOutlined sx={{ fontSize: 26 }} />
              ) : isPending ? (
                <ScheduleOutlined sx={{ fontSize: 26 }} />
              ) : (
              ...
              return (
                <React.Fragment key={step.id || idx}>
                  <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", zIndex: 2 }}>
                    ...
                    <Typography variant="caption" sx={{ color: "#94A3B8", fontSize: "10px", textAlign: "center" }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 1.8 }}>
            {whatHappensNext.map((item, index) => (
              <Box key={item.id || index} sx={{ display: "flex", alignItems: "flex-start", gap: 1.5 }}>
                <CheckCircle sx={{ fontSize: 16, color: "#017E53", mt: 0.2, flexShrink: 0 }} />
